...
const Search = () => {
  ...
  // Instead of using _from, we use an offset state (initialized to 0)
  ...
  // We'll use this to decide if we have a next page
  ...
  useEffect(() => {
    ...
    dispatch(clearError());
  }, [city, zipCode, states, geoBoundingBox, ageMin, ageMax, sort, size, dispatch]);
    ...
    try {
      ...
      const response = await dispatch(
        getDogs({
          ...
          geoBoundingBox:
            geoBoundingBox.top?.lat && geoBoundingBox.left?.lon
              ? geoBoundingBox
              : undefined,
          ageMin,
          ageMax,
      ...
      // Store total from the API (if provided)
        ...
        <section className="p-4 sm:p-8 max-w-5xl mx-auto bg-white shadow-md rounded-md mt-6">
          ...
          <div className="flex flex-wrap gap-4 items-center justify-center mb-4">
            ...
            <input
              ...
              className="p-2 border rounded w-full sm:w-auto"
              value={geoBoundingBox.bottom?.lat || ""}
              onChange={(e) =>
                setGeoBoundingBox((prev) => ({
                  ...prev,
                  bottom: {
                    lat: Number(e.target.value),
                    lon: prev.bottom?.lon || 0,
                  },
                }))
              }
              onKeyDown={(e) => {
            ...
            <input
              ...
              className="p-2 border rounded w-full sm:w-auto"
              value={geoBoundingBox.right?.lon || ""}
              onChange={(e) =>
                setGeoBoundingBox((prev) => ({
                  ...prev,
                  right: {
                    lat: prev.right?.lat || 0,
                    lon: Number(e.target.value),
                  },
                }))
              }
